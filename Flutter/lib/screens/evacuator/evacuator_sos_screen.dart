...
final _activeEvacProvider = FutureProvider.autoDispose<SosRequest?>((ref) {
  return SosService().getActiveForEvacuator();
});

final _incomingEvacProvider = FutureProvider.autoDispose<List<SosIncomingRequest>>((ref) {
  return SosService().listForEvacuator();
});

class EvacuatorSosScreen extends ConsumerStatefulWidget {
  const EvacuatorSosScreen({super.key});

  @override
  ConsumerState<EvacuatorSosScreen> createState() => _EvacuatorSosScreenState();
}

class _EvacuatorSosScreenState extends ConsumerState<EvacuatorSosScreen> {
  ...
  void initState() {
    ...
    _refreshLocationAndProfile();
  }

  Future<void> _refreshLocationAndProfile() async {
    try {
      final profile = await EvacuatorService().getProfile();
      if (mounted) setState(() => _available = profile.isAvailable);
    } catch (_) {}

    final result = await LocationService().getCurrentPosition();
    if (result.isOk) {
      EvacuatorService().updateLocation(result.position!.latitude, result.position!.longitude).catchError((_) {});
    }
  }
  ...
  Future<void> _toggleAvailability(bool value) async {
    ...
    try {
      await EvacuatorService().setAvailability(value);
      if (value) _refreshLocationAndProfile();
    } catch (_) {
  ...
  void _onWsEvent(WsEvent event) {
    if (event.type == 'sos_dispatch') {
      ref.invalidate(_incomingEvacProvider);
  ...
  Future<void> _accept(SosIncomingRequest item) async {
    ...
    try {
      await SosService().acceptAsEvacuator(item.sosRequestId);
      if (!mounted) return;
      ref.invalidate(_activeEvacProvider);
      ref.invalidate(_incomingEvacProvider);
      context.push('/evacuator/sos/${item.sosRequestId}');
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
  ...
  Widget build(BuildContext context) {
    final activeAsync = ref.watch(_activeEvacProvider);
    final incomingAsync = ref.watch(_incomingEvacProvider);
          ...
          children: [
            Padding(
              ...
              child: Row(
                children: [
                  Expanded(
                    child: Text('Evakuator', style: AppTypography.appbarTitle.copyWith(color: AppColors.text(context))),
                  ),
                  Text(
                    _available ? 'Onlayn' : 'Oflayn',
                    style: AppTypography.labelSmall.copyWith(
                      color: _available ? AppColors.success : AppColors.text3(context),
                    ),
                  ),
                      ...
                      data: (active) => active == null
                          ...
                          : Padding(
                              ...
                              child: _ActiveJobCard(
                                request: active,
                                onTap: () => context.push('/evacuator/sos/${active.id}'),
                              ),
                            ),
                    ...
                    if (!_available)
                      Padding(
                        ...
                        child: Container(
                          ...
                          child: Text(
                            'Siz oflaynsiz — yangi SOS chaqiruvlari kelmaydi',
