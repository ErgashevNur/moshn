// moshn-screens-auth.jsx — onboarding + OTP login

// Brand mark: concentric tire ring (simple geometry)
...
function Onboarding({ app }) {
  ...
  const slides = [
    ...
    { v: "sos",    title: t("ob2Title"), sub: t("ob2Sub") },
  ...
  const Visual = ({ v }) => {
    if (v === "sos") return (
      <div style={{ position: "relative", display: "grid", placeItems: "center", width: 180, height: 180 }}>
        <div style={{ position: "absolute", width: 120, height: 120, borderRadius: "50%", border: "2px solid var(--danger)", animation: "pulse-ring 2s ease-out infinite" }} />
        <div style={{ position: "absolute", width: 120, height: 120, borderRadius: "50%", border: "2px solid var(--danger)", animation: "pulse-ring 2s ease-out infinite 1s" }} />
        <div style={{ width: 92, height: 92, borderRadius: "50%", background: "var(--danger)", display: "grid", placeItems: "center", color: "#fff", boxShadow: "0 10px 30px -6px var(--danger)" }}>
          <span style={{ fontWeight: 800, fontSize: 22, letterSpacing: "0.1em" }}>SOS</span>
        </div>
      </div>
    );
    if (v === "pay") return (
...
function Auth({ app }) {
  ...
  return (
    <div className="screen">
      <AppBar onBack={() => step === "code" ? setStep("phone") : app.go("onboarding")}
              ...
              {code.map((c, idx) => (
                <input key={idx} ref={el => inputs.current[idx] = el} className="mono"
                  ...
                  style={{
                    ...
                    borderRadius: 16, background: "var(--surface)", color: "var(--text)",
                    border: `1.5px solid ${c ? "var(--text-3)" : "var(--hairline)"}`, outline: "none",
                  }} />
              ))}
            </div>
...
Object.assign(window, { Onboarding, Auth, BrandMark, LangToggle });
